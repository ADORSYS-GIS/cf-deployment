# Update Guide for Cloud Foundry version 6.44.1 - 8.7.10
...
The goal of this document is to bring out the new features, fixes and bugs which have occured from the cloud foundry CLI from version ```6.44.1``` - ```8.7.10```, and brings out 
...
#### New feautes supported by cloud foundry version 7 not found found in version 6 (Breaking Changes)
...
- ***Running cloud foundry sub-step commands*** these commands breakdown the cloud foundry push process into sub steps that can run independently.
...
| ```cf apps```                           |                                        | Displays information about ```processes```.<br> ```url``` field renamed to ```routes```, removed ```instances```, ```memory```and ```disk``` is removed.<br> Apps are listed alphabetically |                                                              |
| ```cf bind-security-group```            |                                        | ```SPACE``` is no longer an argument; use the ```--space``` flag                                                   |                                                              |
| ```cf check-route```                    | ```--hostname```                             | ```HOST``` is no longer a required argument.<br> no longer requires a backslash                                   |                                                              |
| ```cf create-buildpack```               |                                        |Creating a buildpack with position set to ```0``` is no longer |```--enable``` Creating a buildpack activates it by default.<br>                                          , ```--disable``` you cannot deactivate a build pack upon creation.                                    |
...
cloud foundry CLI v8 uses CAPI v3 to make requests to services, CAPI v3 creates asynchrounous jobs if you want to continue to create jobs asynchrounouly use the ```--wait``` flag

### Table of Differences in version 8.7.10
|command|Added/New flag | updates | Remove | Script Adjustments|
|-------|---------------|---------|--------|-------------------|
|```cf bind-service```|```--wait``` for the bind operation to complete|||Handle asynchronous operations by adding --await for the bind of cloud foundry services|
|```cf bind-route-service```|```--wait``` to wait for the create operation to complete.|||Handle bind route operation in async as before.<br>Bind route operation is asynchronous by default|
|```cf create-service```|```--wait``` to wait for the create operation to complete|||create operation should await using --await|
|```cf create-service-key```|```--wait``` to wait for the delete operation|||Delete operation is async by delete|
|```cf delete-serivce```|```--wait``` to wait for the delete operation to complete|||Script should make delete operation asynchrounous by default|
|```cf delete-service-key```|```--wait``` to wait for the delete operation to complete|||delete operation should be async by default|
...
|```cf service```|use ```--params```to retrieve and display the given service instance's parameters as JSON. All other outputs is suppressed|displays info **guid**, **type**, and **broker tags**<br>**dashboard** field is renamed to **dashboard url**<br>. Minor changes to the ordering and wording of each block of information.||**--params** should display the given service instance's parameters as JSON<br>.should display info about guid, type and broker tags<br>. Update the service broker field|
...
|```cf services```| use ```--no-apps``` to not retrieve bound apps information<br>```--wait``` to wait for operations|||CLI should include ```--no-apps``` option|
...
|```cf unbind-route-service```|Use ```--wait``` to wait for the unbind operation to complete.|||make unbind operation asynchronous by default.|
